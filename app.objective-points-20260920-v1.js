...
const EXTREME_POINT_LABEL_HEIGHT = 22;
const EXTREME_POINT_LABEL_GAP = 9;
const EXTREME_POINT_LABEL_EDGE_MARGIN = 7;
...
const dom = {
  ...
  lockViewInline: document.getElementById("lock-view-inline"),
...
const state = {
  ...
  viewSettings: { equalUnits: true, locked: false },
  objective: {
    mode: "max",
    xCoeff: "0.75",
    yCoeff: "1",
...
function toggleGraphViewLock() {
  state.viewSettings.locked = !state.viewSettings.locked;

  if (state.viewSettings.locked) {
    autoFitViewToConstraints(state.constraints);
  } else {
    viewNotice = "Graph view unlocked. Drag the background to pan or use the mouse wheel to zoom.";
  }
...
function bindStaticEvents() {
  ...
  dom.lockViewInline.addEventListener("click", toggleGraphViewLock);
...
function handlePlotPointerDown(event) {
  ...
  if (dragKind === "view-pan" && state.viewSettings.locked) {
    return;
  }
...
function handlePlotWheel(event) {
  if (state.viewSettings.locked || dragState || event.deltaY === 0) {
    return;
  }
...
function syncGraphLockUi() {
  const locked = state.viewSettings.locked;
  dom.lockViewInline.setAttribute("aria-pressed", String(locked));
  dom.lockViewInline.classList.toggle("is-locked", locked);
  dom.graphPanel.dataset.viewLocked = String(locked);
}
...
function getAnalysis() {
  ...
  const extremePoints = findFeasibleExtremePoints(halfPlanes);
  const objective = getObjectiveCoefficients();
  ...
  const objectiveBoundaryIntersections = findObjectiveBoundaryIntersections(objectiveLine, halfPlanes);
  ...
  analysisCache = {
    ...
    currentLevel,
    objectiveLine,
    objectiveBoundaryIntersections,
    currentLineSegment,
    currentContacts,
    objectiveRange,
    optimization,
...
function renderPlot(analysis) {
  dom.plot.innerHTML = "";
  const pointLabels = getPointLabelLayout(analysis.extremePoints, analysis.objectiveBoundaryIntersections, analysis.view);
  syncPlotAccessibleLabel(pointLabels);
  ...
  drawPointLabels(layers.labels, pointLabels);
...
function drawPanSurface(group) {
  ...
  group.appendChild(
    svgElement("rect", {
      ...
      ...(locked ? {} : { "data-drag-kind": "view-pan" }),
      "data-cursor": locked ? "default" : "grab",
...
function drawPointLabels(group, layouts) {
  layouts.forEach((layout) => {
    const labelGroup = svgGroup();
    if (layout.isExtremePoint) labelGroup.setAttribute("data-extreme-point", "");
    if (layout.isObjectiveIntersection) labelGroup.setAttribute("data-objective-intersection", "");
    const color = layout.isObjectiveIntersection ? "#8F5F00" : "#0E5C51";
    labelGroup.setAttribute("data-x", formatNumber(layout.point.x));
    labelGroup.setAttribute("data-y", formatNumber(layout.point.y));
    labelGroup.setAttribute("aria-hidden", "true");

    labelGroup.appendChild(
      svgElement("line", {
        x1: layout.svgPoint.x,
        y1: layout.svgPoint.y,
        x2: layout.labelX,
        y2: layout.labelY,
        stroke: color,
        "stroke-opacity": 0.48,
    ...
    labelGroup.appendChild(
      svgElement("circle", {
        cx: layout.svgPoint.x,
        cy: layout.svgPoint.y,
        r: layout.isObjectiveIntersection ? 6 : 4.5,
        fill: layout.isObjectiveIntersection ? "#F5A623" : "#fffaf2",
        stroke: layout.isObjectiveIntersection ? "#FFF9E9" : color,
        "stroke-width": 2,
        "pointer-events": "none",
      })
    );
...
function getExtremePointLabelLayout(extremePoints, view) {
  return getPointLabelLayout(extremePoints, [], view);
}

function getPointLabelLayout(extremePoints, objectivePoints, view) {
  const points = extremePoints.map((point) => ({ point, isExtremePoint: true, isObjectiveIntersection: false }));
  objectivePoints.forEach((point) => {
    const existing = points.find((entry) =>
      Math.abs(entry.point.x - point.x) <= 1e-5 && Math.abs(entry.point.y - point.y) <= 1e-5
    );
    if (existing) {
      existing.isObjectiveIntersection = true;
    } else {
      points.push({ point, isExtremePoint: false, isObjectiveIntersection: true });
    }
  });
  const visiblePoints = points
    .filter((entry) => isPointInView(entry.point, view))
    .map((entry) => ({
      ...entry,
      svgPoint: worldToSvg(entry.point.x, entry.point.y, view),
      text: formatExtremePointLabel(entry.point),
    }))
    .sort((first, second) =>
      Math.abs(first.svgPoint.y - second.svgPoint.y) > EPSILON
        ? first.svgPoint.y - second.svgPoint.y
        : first.svgPoint.x - second.svgPoint.x
    );
  ...
  const centroid = visiblePoints.reduce(
    (total, entry) => ({ x: total.x + entry.svgPoint.x, y: total.y + entry.svgPoint.y }),
    { x: 0, y: 0 }
  );
  centroid.x /= visiblePoints.length;
  centroid.y /= visiblePoints.length;

  const placedBounds = [];
  return visiblePoints.map((entry) => {
    const width = clamp(54, entry.text.length * 7.2 + 18, 240);
    const height = EXTREME_POINT_LABEL_HEIGHT;
    const horizontal = Math.abs(entry.svgPoint.x - centroid.x) <= 1
      ? (entry.svgPoint.x <= PLOT_BOX.x + PLOT_BOX.width / 2 ? 1 : -1)
      : Math.sign(entry.svgPoint.x - centroid.x);
    const vertical = Math.abs(entry.svgPoint.y - centroid.y) <= 1
      ? (entry.svgPoint.y <= PLOT_BOX.y + PLOT_BOX.height / 2 ? 1 : -1)
      : Math.sign(entry.svgPoint.y - centroid.y);
    const directions = [
      [horizontal, vertical],
      [horizontal, 0],
      [0, vertical],
    ...
    const candidates = directions.map(([dx, dy], index) => {
      const magnitude = Math.hypot(dx, dy) || 1;
      const unitX = dx / magnitude;
      const unitY = dy / magnitude;
      const horizontalDistance = Math.abs(unitX) > EPSILON
        ? (width / 2 + EXTREME_POINT_LABEL_GAP) / Math.abs(unitX)
        : Number.POSITIVE_INFINITY;
      const verticalDistance = Math.abs(unitY) > EPSILON
        ? (height / 2 + EXTREME_POINT_LABEL_GAP) / Math.abs(unitY)
        : Number.POSITIVE_INFINITY;
      const distance = Math.min(horizontalDistance, verticalDistance);
      const rawX = entry.svgPoint.x + unitX * distance;
      const rawY = entry.svgPoint.y + unitY * distance;
      const labelX = clamp(
        PLOT_BOX.x + width / 2 + EXTREME_POINT_LABEL_EDGE_MARGIN,
        rawX,
        PLOT_BOX.x + PLOT_BOX.width - width / 2 - EXTREME_POINT_LABEL_EDGE_MARGIN
      );
      const labelY = clamp(
        PLOT_BOX.y + height / 2 + EXTREME_POINT_LABEL_EDGE_MARGIN,
        rawY,
        PLOT_BOX.y + PLOT_BOX.height - height / 2 - EXTREME_POINT_LABEL_EDGE_MARGIN
      );
      const bounds = {
        left: labelX - width / 2,
      ...
      const overlapArea = placedBounds.reduce(
        (total, existing) => total + rectangleOverlapArea(bounds, existing, 4),
        0
      );
      const coversPoint = pointInsideBounds(entry.svgPoint, bounds, 5);
      const clampDistance = Math.hypot(labelX - rawX, labelY - rawY);
      return {
        labelX,
        labelY,
        bounds,
        score: overlapArea * 100 + (coversPoint ? 100000 : 0) + clampDistance * 20 + index,
      };
    });
    ...
    return { ...entry, ...placement, width, height };
  });
}

function isPointInView(point, view) {
  const xTolerance = Math.max(EPSILON, (view.xMax - view.xMin) * 1e-9);
  const yTolerance = Math.max(EPSILON, (view.yMax - view.yMin) * 1e-9);
  return point.x >= view.xMin - xTolerance && point.x <= view.xMax + xTolerance &&
    point.y >= view.yMin - yTolerance && point.y <= view.yMax + yTolerance;
}

function rectangleOverlapArea(first, second, padding = 0) {
  const width = Math.max(0, Math.min(first.right, second.right + padding) - Math.max(first.left, second.left - padding));
  const height = Math.max(0, Math.min(first.bottom, second.bottom + padding) - Math.max(first.top, second.top - padding));
  return width * height;
}
...
function formatExtremePointLabel(point) {
  return `(${formatResultPrimary(point.x)}, ${formatResultPrimary(point.y)})`;
}

function syncPlotAccessibleLabel(layouts) {
  const base = "2D graph showing linear constraints, the feasible region, the objective function, and its improving direction.";
  const extremePoints = layouts.filter((layout) => layout.isExtremePoint);
  const objectivePoints = layouts.filter((layout) => layout.isObjectiveIntersection);
  const summary = extremePoints.length
    ? ` Visible extreme points: ${extremePoints.map((layout) => layout.text).join(", ")}.`
    : " No extreme points are visible in the current window.";
  const objectiveSummary = objectivePoints.length
    ? ` Visible objective intersections with the feasible boundary: ${objectivePoints.map((layout) => layout.text).join(", ")}.`
    : " No objective intersections with the feasible boundary are visible in the current window.";
  dom.plot.setAttribute("aria-label", `${base}${summary}${objectiveSummary}`);
}
...
function autoFitViewToConstraints(constraints) {
  ...
  viewNotice = fit.kind === "feasible"
    ? state.viewSettings.locked
      ? "Graph view fitted and locked. Constraints and the objective remain draggable."
      : "Fitted to the feasible region. The view stays fixed while you edit or drag."
    : fit.kind === "empty"
      ? state.viewSettings.locked
        ? "Graph view locked to the default window because there are no active constraints."
        : "No active constraints. Showing the default window."
      : state.viewSettings.locked
        ? "Graph view locked to the constraint overview. Unlock to pan or zoom, or set axis ranges."
...
function computeAutomaticView(constraints, equalUnits = true) {
  ...
  if (isRegionBounded(halfPlanes)) {
    const vertices = findFeasibleExtremePoints(halfPlanes);
    if (vertices.length) {
...
function findFeasibleExtremePoints(halfPlanes) {
  const points = [];
  for (let index = 0; index < halfPlanes.length; index += 1) {
    for (let otherIndex = index + 1; otherIndex < halfPlanes.length; otherIndex += 1) {
      const point = lineLineIntersection(halfPlanes[index].line, halfPlanes[otherIndex].line);
...
function findObjectiveBoundaryIntersections(line, halfPlanes) {
  if (!line || Math.hypot(line.a, line.b) <= EPSILON) return [];
  // Use real constraint boundaries so panning/zooming cannot invent intersections
  // at the viewport edges. Coincident edges contribute their finite endpoints via
  // the other boundaries; shared vertices are labeled only once.
  return dedupePoints(halfPlanes
    .map((halfPlane) => lineLineIntersection(line, halfPlane.line))
