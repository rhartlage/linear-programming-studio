...
function renderPlot(analysis) {
  dom.plot.innerHTML = "";
  const extremePointLabels = getExtremePointLabelLayout(analysis.extremePoints, analysis.view);
  syncPlotAccessibleLabel(extremePointLabels);
  ...
  drawExtremePointLabels(layers.labels, extremePointLabels);
...
function drawExtremePointLabels(group, layouts) {
  layouts.forEach((layout) => {
    const labelGroup = svgGroup();
    labelGroup.setAttribute("data-extreme-point", "");
...
function getExtremePointLabelLayout(extremePoints, view) {
  const visiblePoints = extremePoints
    .filter((point) => isPointInView(point, view))
    .map((point) => ({
      point,
      svgPoint: worldToSvg(point.x, point.y, view),
      text: formatExtremePointLabel(point),
    }))
    .sort((first, second) =>
...
function syncPlotAccessibleLabel(layouts) {
  ...
  const summary = layouts.length
    ? ` Visible extreme points: ${layouts.map((layout) => layout.text).join(", ")}.`
    : " No extreme points are visible in the current window.";
  dom.plot.setAttribute("aria-label", `${base}${summary}`);
}
